...
async function getSharedLinkData() {
  try {
    const response = await $fetch(`/api/shared_link?share=${route.params.id.toString()}`)
    ...
    errorMsg.value = "An unexpected error occurred. Please contact the developer. (Error code: 84)"
  }
}
...
<template>
    ...
    <v-card class="h-50">
      ...
      <v-card-text style="overflow-y: auto;">
        ...
        <ul v-auto-animate>
          ...
          <v-list-item v-for="(shared_item, index) in shared_data" :key="index" :href="'https://twitch.tv/' + shared_item"
            target="_blank" ref="dialog_shared_list">
            <v-btn variant="plain" icon="mdi-delete" color="red-accent-2"
              @click="(event: MouseEvent) => removeShareItem(index, event)"></v-btn>
            {{ shared_item }}
          </v-list-item>
        </ul>
        <br>
        ...
        <ul>
          <v-list-item v-for="(octo_item, index) in octoStore.octoData" :key="index"
            :href="'https://twitch.tv/' + octo_item" target="_blank">
            {{ octo_item }}
          </v-list-item>
        </ul>
      </v-card-text>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="deep-purple-darken-1" variant="flat" @click="handleCancel">
          Cancel
        </v-btn>
        <v-tooltip location="top">
          <template v-slot:activator="{ props }">
            <v-btn color="deep-purple-lighten-2" variant="text" @click="dialog2 = !dialog2" v-bind="props">
              Replace
            </v-btn>
          </template>
  ...
  <v-dialog v-model="dialog2" width="auto" max-width="400px">
    <v-card>
      ...
      <v-card-text>
        <p>
          This will delete your existing list and replace it with the shared list
        </p>
        <br>
        <h4>
        ...
        <br>
        <v-btn color="deep-purple-lighten-2" variant="text" @click="handleReplace">
          I'm Sure
        </v-btn>
      </v-card-text>
      <v-divider></v-divider>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="deep-purple-darken-1" variant="flat" @click="dialog2 = false">
          Close
        </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
  <v-snackbar :timeout="2000" color="green-darken-2" variant="tonal" v-model="snackbar" rounded="pill">
